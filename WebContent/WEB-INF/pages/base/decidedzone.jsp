...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length != 1){
			...
			//发送ajax请求，请求Action，在action中调用crm代理对象，
			//通过代理对象远程访问crm服务获取客户数据，在Action中将客户数据转为json，响应到客户端浏览器
			$("#noassociationSelect").empty();
			$("#associationSelect").empty();
			var url_1 = "${pageContext.request.contextPath}/decidedzoneAction_findnoassociationCustomers.action";
			...
			//获得选中的定区id
			var did = rows[0].id;
			var url_2 = "${pageContext.request.contextPath}/decidedzoneAction_findhasassociationCustomers.action";
			$.post(url_2,{"id":did},function(data){
				for(var i=0;i<data.length;i++){
					var id = data[i].id;
					var name = data[i].name;
	...
	function doDblClickRow(a,b){
		...
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "json/association_subarea.json?id="+b.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
